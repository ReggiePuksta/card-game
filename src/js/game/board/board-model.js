...
BoardModel.prototype.getAllAvailableSlots = function() {
  this.getAvailableCreatureSlots();
  this.getAvailableTrapSlots();
};
BoardModel.prototype.getAvailableCreatureSlots = function(card) {
  for (var i = 0, len = this.board.length; i < len; i++) {
    
  }
};
BoardModel.prototype.getAvailableTrapSlots = function(card) {

};
boardmodel.prototype.getAvailableSlots = function(cardId) {
  var card = this.cardsStorage.get(cardId);
  var type = card.model.type;
  switch (type) {
    case 'creature':
      this.getAvailableCreatureSlots(card);
      break;
    case 'trap':
      this.getAvailableTrapSlots(card);
      break;
    case 'ongoing':
...
boardmodel.prototype.getCardInsideSlot= function(slotId) {
  var cardId = this.board[slotId];
  if (!cardId) {
    return 0;
  }
  return this.cardsStorage.get(cardId);
};
